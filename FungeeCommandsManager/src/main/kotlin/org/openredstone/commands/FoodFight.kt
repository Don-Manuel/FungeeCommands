...
class FoodFight(val plugin: FungeeCommandsManager) : FunCommand(plugin, "foodfight") {
    ...
    private val randomFood: String
        get() = foods[rand.nextInt(foods.size)]

    private fun prettifyFood(food: String): String {
        val words = food.split("_").toTypedArray()
    ...
    companion object {
        private fun capitalizeWord(word: String): String {
            return Character.toUpperCase(word[0]).toString() + word.substring(1).toLowerCase()
        }
    }
}
